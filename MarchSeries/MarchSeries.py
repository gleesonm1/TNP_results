from zpdatafetch import Result, Primes, Sprints
import json
import pandas as pd
import numpy as np
import asyncio
...
## set up information
round = {'Round 1': [5455291,5455294],
         'Round 2': [5455295,5455297],
         'Round 3': [5455298,5455299],
         'Round 4': [5455301,5455302],
         'Round 5': [5455303]}
...
## assign points to various segments
    ...
    for idx, key in enumerate(round):
        ...
        if len(race_id) > 0:
            for i in race_id:
                ...
                race_data = await result.afetch(i)
...
# calculate offsets and number of races completed
...
egap_cols = ['pen', 'category', 'zwift_id', 'name', 'team_name', 'egap']
columns = ['pen', 'category', 'zwift_id', 'name', 'team_name', 'time', 'egap']
...
for idx, key in enumerate(round):
    ...
    if len(race_id) > 0:
        for i in race_id:
            ...
            gc_filtered = gc[gc['races'] >= 0].copy() # >=2
            pen_map = gc_filtered.set_index('zwift_id')['pen']
            mask = result['zwift_id'].isin(gc_filtered['zwift_id'])
            result.loc[mask, 'pen'] = result.loc[mask, 'zwift_id'].map(pen_map) # reset pen to overall pen
            result = result.loc[mask]

            result['egap'] = result['time'] - result.groupby('pen')['time'].transform('min')
        ...
        # if duplicates only take lowest time
        res = res.loc[res.groupby('zwift_id')['egap'].idxmin()].reset_index(drop=True)

        res.sort_values(by = ['pen', 'egap'], ignore_index=True, inplace = True)

        res = res.rename(columns={'egap': 'egap'+str(idx+1)})
        res = res.rename(columns={'time': 'time'+str(idx+1)})
        
        egap_cols.append('egap'+str(idx+1))
        egap_cols.append('time'+str(idx+1))
        
    egap[key] = res

all_rounds = [
    df[df.columns.intersection(egap_cols)] # Only take columns that exist in THIS df
    for key, df in egap.items()
    if key != 'GC' and not df.empty
]

all_egap_rows = pd.concat(all_rounds, ignore_index=True)
time_cols = [c for c in all_egap_rows.columns if c.startswith('time')]
egap_cols = [c for c in all_egap_rows.columns if c.startswith('egap')]
assert all_egap_rows.groupby('zwift_id')[egap_cols].count().max().max() <= 1
...
egap_fin = (
    egap_base
      .merge(egap_pen, on='zwift_id', how='left')
      .sort_values(by='pen', key=lambda s: s.map(pen_rank), ignore_index=True)
)

# calculate offsets and number of races completed
egap_fin['races'] = egap_fin[time_cols].count(axis=1)
egap_fin['total_time'] = egap_fin[time_cols].sum(axis=1)
egap_fin['egap'] = egap_fin[egap_cols].sum(axis=1)

egap_fin.sort_values(by = ['pen', 'races', 'egap'], ascending = [True, False, True], 
               ignore_index=True, inplace = True)

egap_fin = egap_fin[['pen', 'category', 'name', 'team_name', 'egap', 'races']]

#### TEAM GC ######
teams = out['Round 1']['team_name'].unique()
...
if len(all_rounds) > 0:
    ...
    gc_team['races'] = gc_team[time_cols].count(axis=1)
    gc_team['total_time'] = gc_team[time_cols].sum(axis=1)

    gc_team['time_offset'] = np.zeros(len(gc_team))
    gc_team.loc[gc_team['races'] == gc_team['races'].max(),'time_offset'] = gc_team['total_time'] - gc_team.loc[gc_team['races'] == gc_team['races'].max()].groupby('pen')['total_time'].transform('min')
    gc_team.loc[gc_team['races'] < gc_team['races'].max(), 'time_offset'] = np.nan

    gc_team.sort_values(by = ['pen', 'races', 'total_time'], ascending = [True, False, True], 
                ignore_index=True, inplace = True)

    team_gc['GC'] = gc_team[['pen', 'team_name', 'total_time', 'time_offset', 'races']]
else:
    ...
    for idx, key in enumerate(round):
        ...
        if len(race_id) > 0:
            for i in race_id: 
                # Get FTS data
                prime = Sprints()
                print(f"Getting KQOM and Sprint data for race {i}")
                prime_data = await prime.afetch(i)

                data_dict = json.loads(prime.json())
                
                race_sprint[i] = data_dict[str(i)]['data']
...
if __name__ == "__main__":
    asyncio.run(prime_data_scrap())

sprints = [x for x in names if x in sprint_list or 'Sprint' in x]
kqoms = [x for x in names if x in KQOM_list or 'KOM' in x]
for x in names:
    if "Sprint" in x:
        sprint_list.update({x: list(range(20,-1,-1))})
    if "KOM" in x:
        KQOM_list.update({x: list(range(20,-1,-1))})
sprint[sprints] = 0.0
KQOM[kqoms] = 0.0

sprint = sprint.loc[sprint['races'] == np.nanmax(sprint['races'])].reset_index(drop = True)
KQOM = KQOM.loc[(KQOM['races'] == np.nanmax(KQOM['races'])) | 
                ((KQOM['races'] == np.nanmax(KQOM['races'])-1) & 
                 (~KQOM['zwift_id'].isin(out['Round 1']['zwift_id'])))].reset_index(drop = True)
...
### Copy/paste to excel
with pd.ExcelWriter("London_Watopia.xlsx") as writer:
